...
//!
//! This example demonstrates Relation PlannerContext methods:
//! - plan(): Recursively plan the base table without the sample clause
        ...
        match relation {
            TableFactor::Table {
                ...
                let sampleless_relation = TableFactor::Table {
                    ...
                    with_ordinality: with_ordinality.clone(),
                ...
                if let Some(bucket) = sample.bucket {
                    ...
                    let plan =
                        TableSamplePlanNode::new(input, value, None, seed).as_plan();
                ...
                return plan_err!("Cannot plan sample SQL");
            }
...
async fn main() -> Result<()> {
    ...
    run_example(
        ...
        "SELECT t1.column1, t2.column1, t1.column2, t2.column2 FROM sample_data t1 TABLESAMPLE (0.7) JOIN sample_data t2 TABLESAMPLE (0.7) ON t1.column1 = t2.column1",

    )
    .await?;

    println!("\nKey Concepts:");
    println!("   - Custom planners can implement SQL sampling extensions");
    println!("   - TABLESAMPLE reduces data volume for faster query prototyping");
    println!("   - Useful for exploratory data analysis on large tables");
    println!();
    println!("Trait methods demonstrated:");
    println!("   - plan(): Used to plan the base table without sampling");
    println!();
    println!("Use cases:");
    println!("   - Quick data exploration on large datasets");
    println!("   - Testing queries before full table scan");
    println!("   - Statistical analysis on representative samples");
