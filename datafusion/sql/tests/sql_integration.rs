...
use datafusion_sql::{
    parser::DFParser,
    planner::{
        NullOrdering, ParserOptions, PlannerContext, RelationPlanner,
        RelationPlannerDelegate, SqlToRel,
    },
};
...
impl RelationPlanner<MockContextProvider> for DelegatingPlanner {
    fn plan_relation(
        &self,
        relation: &TableFactor,
        _sql_to_rel: &SqlToRel<'_, MockContextProvider>,
...
struct DefaultPassthroughPlanner;

impl RelationPlanner<MockContextProvider> for DefaultPassthroughPlanner {
    fn plan_relation(
        &self,
        relation: &TableFactor,
        _sql_to_rel: &SqlToRel<'_, MockContextProvider>,
...
struct DefaultOnlyPlanner;

impl RelationPlanner<MockContextProvider> for DefaultOnlyPlanner {
    fn plan_relation(
        &self,
        relation: &TableFactor,
        _sql_to_rel: &SqlToRel<'_, MockContextProvider>,
...
#[test]
fn relation_planner_chain_uses_next() -> Result<()> {
    let context = MockContextProvider {
        state: MockSessionState::default(),
    };
    let planners: Vec<Arc<dyn RelationPlanner<MockContextProvider> + Send + Sync>> = vec![
        Arc::new(DelegatingPlanner),
        Arc::new(DefaultPassthroughPlanner),
    ];
    let planner = SqlToRel::new(&context).with_relation_planners(planners);
    let statement = DFParser::parse_sql("SELECT * FROM person")?
        .into_iter()
        .next()
        .unwrap();
    let plan = planner.statement_to_plan(statement)?;
...
#[test]
fn relation_planner_uses_default_when_no_next() -> Result<()> {
    let context = MockContextProvider {
        state: MockSessionState::default(),
    };
    let planners: Vec<Arc<dyn RelationPlanner<MockContextProvider> + Send + Sync>> =
        vec![Arc::new(DefaultOnlyPlanner)];
    let planner = SqlToRel::new(&context).with_relation_planners(planners);
