...
pub struct ConvertOpt {
    ...
    /// Sort each table by its first column (the "key" column) in ascending order.
    ...
    pub async fn run(self) -> Result<()> {
        ...
        for table in TPCH_TABLES {
            ...
            let options = if self.sort {
                // indicated that the file is already sorted by the key column to speed up the conversion
                options.file_sort_order(vec![vec![
                    col(key_column_name).sort(true, false)
                ]])
            } else {
                options
            };
